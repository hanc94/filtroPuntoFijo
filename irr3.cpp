...
delay_t iir3 ( coeff_t x_n,delay_t *v, const coeff_t *b, const coeff_t *a, int8_t n,int8_t q){
	int8_t k;
...
/*Cálculo de v[n]*/
	for (k = 0; k < n - 2; k++)
		v[k] = ((acc_t)b[k+1]*((acc_t)x_n)) - (((acc_t)a[k + 1]) *((acc_t)y_n) + ((acc_t)v[k+1]);
/*Cálculo de v_{n-1}*/
	v[ n - 2] = ((acc_t)b[n-1]) * ((acc_t)x_n)-((acc_t)a [ n - 1]) * ((acc_t)y_n);
...
/* Puntero a la matriz de salida */
	y = (delay_t*)mxGetPr(plhs[0]);

	delay_t v[((mwSize)p)-1]={0.0};

	for (int i=0;i<(mwSize)nx;i++){
		y[i]=iir3(xn[i],v,b,a,(mwSize)p,q);
	}
